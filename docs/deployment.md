...
- Production URL will be `https://<github-username>.github.io/<repo>/` — **PENDING** the GitHub username/repo name.
- This URL must be baked into the extension before publishing: set the extension's `PLANNER_URL` / `PLANNER_ORIGIN` constant and the planner-bridge content-script `matches` + `host_permissions` to this domain (currently `http://localhost:5173` placeholder).
