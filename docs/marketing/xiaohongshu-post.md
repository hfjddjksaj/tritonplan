# 小红书首发笔记（可直接复制发布）

> 2026-07 制作。图片素材：本目录 `xiaohongshu/` 两张生成卡 + `docs/screenshots/` 产品图。
> Reddit 主帖等 v0.1.1 上架后再发（草稿见 `../marketing-plan.md` 4.1 节）。

## 图片顺序（5 张）

1. `xiaohongshu/xhs-cover.png` — 大字封面（3:4）
2. `../screenshots/calendar.png` — 周历冲突图
3. `xiaohongshu/xhs-steps.png` — 三步上手卡（3:4，内嵌 TSS 按钮实拍）
4. `../screenshots/finals.png` — Finals 撞车图
5. `../screenshots/building-popover.png` — 教学楼弹窗（彩蛋功能）

## 标题（≤20 字，选一）

- UCSD新系统没课表？我把它做回来了
- UCSD选课神器｜WebReg课表回来了
- 被TSS折磨的UCSD人 快用这个排课

## 正文

UCSD 新系统 TSS 把老 WebReg 的周历排课砍了，选课全靠脑补时间表，final 撞没撞车要自己翻着算……

于是我做了个免费工具 TritonPlan🔱：在 TSS 里正常浏览课程，点一下卡片上的 "+ TritonPlan"，课就落进 Google Calendar 式的周历——

✅ 时间冲突自动标红，一眼看见哪两门打架
✅ Final 撞车提前预警（选课前就知道！）
✅ 换个 section 组合，红块当场消掉
✅ 一条链接把课表分享给朋友拼课
✅ 点教学楼名直接弹位置，跳 Google Maps

安全这块直接说清楚：插件是纯只读的，不往学校服务器发任何请求、不碰你的账号、不代你点任何按钮，流量和正常浏览一模一样；没有后端没有账号体系，课表只存在你自己浏览器里。全部代码开源可查。

Chrome / Edge 都能用，电脑上打开扩展商店搜 TritonPlan，唯一一个结果就是它；装好点图标直接进排课日历。

新生们 orientation 选课前装上，秋季课表提前排明白；老生们 11 月 Winter 选课见真章。

（UCSD 学生自制，非官方工具，有 bug 评论区喊我）

## 标签

#UCSD #加州大学圣地亚哥分校 #美国留学 #选课 #留学生生活 #新生攻略 #美本 #ucsd选课

## 发布后动作

1. **第一条评论**（发布后立刻，自己顶起来）——**全程零 URL**，只给搜索指令：
   > 📥 安装：电脑 Chrome / Edge 打开扩展商店，搜 TritonPlan——唯一结果，金色三叉戟图标
   > 装好后点浏览器右上角的扩展图标，就能打开排课日历
   > 代码在 GitHub 同名仓库开源，较真的可以去搜来看
   > 手机刷到的先收藏，回电脑装～
2. 24 小时内盯评论区：问安全性 → 用"零请求/开源/无账号"三点事实回；报 bug → 快修快回。
3. 有人私信要链接 → 一对一回复时可以发（被动、低频的私信风险低），不主动群发。
4. 数据参考：收藏 > 点赞 说明选对了人群（收藏 = 选课时要用）。

## ⚠️ 外链红线（2026-07 核查小红书规则后定的铁律）

- **任何场景都不出现 URL**：正文、评论区、个人简介、置顶，一律不放链接/二维码/口令。社区公约明文禁止"在个人页、**评论**、私信等场景引导站外"，机审识别的就是 URL 模式本身，不管你是不是免费工具；后果是评论被折叠成"仅自己可见"+ 笔记不进推荐流，新号尤其敏感。
- 唯一合规引导 = **搜索指令**（"商店搜 TritonPlan"），图片卡底栏已是这个写法。
- 漏斗闭环不依赖网站 URL：商店搜到 → 装扩展 → 点扩展图标 → 直达排课网站。
- GIF 在图文笔记会变静图，暂不用；后续可把 section-switch 做成视频笔记二发。
