...
import { loadPlan, savePlan, loadPool, savePool } from '../lib/storage';
...
// Demo seed: stands in for courses the student has "browsed" in TSS. The real
// extension re-pushes the pool on each load via a `courses` bridge message.
const SAMPLE = sampleCourses as unknown as CourseOffering[];
...
/** Seed the browsed pool from the demo sample + anything persisted from a prior session. */
function initialPool(): CourseOffering[] {
  return mergeCourses(SAMPLE, loadPool() ?? []);
}
...
export function usePlan() {
  ...
  const conflictedCourseIds = useMemo(() => {
    ...
    for (const c of [...weeklyConflicts, ...finalConflicts]) {
      ids.add(c.aCourseId);
      ids.add(c.bCourseId);
    }
    return ids;
