...
describe('storage round-trip', () => {
  ...
  it('round-trips a plan with legacy `hidden`, and rejects a non-array `hidden`', async () => {
    const { savePlans, loadPlans } = await import('./storage');
    localStorage.clear();
    // `hidden` is legacy migration input — still valid stored data, built by hand.
    const base = seeded();
    const s = {
      ...base,
      plans: base.plans.map((p) =>
        p.id === base.activeId ? { ...p, hidden: ['CSE-008A|2026|2'] } : p,
      ),
    };
    savePlans(s);
    expect(loadPlans()).toEqual(s);
